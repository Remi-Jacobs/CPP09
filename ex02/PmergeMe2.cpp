...
//     std::cout << "This da main chain "; 
//     for(std::vector<unsigned int>::iterator it = main_chain.begin(); it != main_chain.end(); ++it)
//         std::cout << *it << " ";
//     std::cout << std::endl;
...
std::vector<unsigned int> PmergeMe::fordJohnson(std::vector<unsigned int> &Arg) {
	...
	for (size_t k = 0; k < pend_order.size(); ++k)
	{
		...
		auto insert_pos = std::lower_bound(main_chain.begin(), main_chain.begin() + std::min(limit_idx + 1, main_chain.size()), val);
